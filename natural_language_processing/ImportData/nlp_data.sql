...
from (
    ...
    from ( 
        ...
        select id from product where Published = 1 and Deleted = 0 
    ) TT 
    on T.id=TT.id
) TTT
    left join product p on p.id=TTT.productId
    left join Ster_Unique_Group_Combination ugc on ugc.ProductId=p.id
    left join Ster_Product_Specs_Mapping psm on psm.productid=p.id

select id, name from product where name like '%[?][?][?]%' or name like '%[?]_[?]_[?]%'

select id, name from product where name like '%???%'
-- ????? 
